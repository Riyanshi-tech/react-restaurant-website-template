...
import { Users, ChefHat, ShoppingBag, DollarSign, QrCode, Loader2 } from 'lucide-react';
...
const AdminHome: React.FC = () => {
  ...
  const cards = [
    ...
    { label: 'Menu Items', value: stats?.menuItems ?? '—', sub: 'In inventory', icon: ChefHat },
    { label: 'Tables', value: stats?.tables ?? '—', sub: `${stats?.occupiedTables ?? 0} occupied`, icon: QrCode },
    { label: 'Open Orders', value: stats?.openOrders ?? '—', sub: 'Unpaid / active', icon: ShoppingBag },
    { label: "Today's Sales", value: stats ? formatINR(stats.todaySales) : '—', sub: `${stats?.todayOrderCount ?? 0} paid orders`, icon: DollarSign },
  ];
